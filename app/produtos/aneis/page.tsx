...
export default function AneisPage() {
  // Filtrar apenas anéis
  const produtosAneis = produtos.filter(
    (p) => p.categoria?.toLowerCase().includes("an") || p.nome.toLowerCase().includes("anel"),
  )

  const [mobileFiltersOpen, setMobileFiltersOpen] = useState(false)
  const [produtosFiltrados, setProdutosFiltrados] = useState(produtosAneis)
  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative h-[40vh] w-full">
        <Image
          src="/placeholder.svg?height=600&width=1600&text=Anéis"
          alt="Anéis CYGA"
          fill
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center text-white p-4">
          <h1 className="text-4xl md:text-6xl font-light mb-4">ANÉIS</h1>
          <p className="text-xl md:text-2xl font-light max-w-2xl">Elegância para suas mãos</p>
        </div>
      </section>
